...
namespace FreeNet
{
    class CListener
    {
        ...
        // 새로운 클라가 접속했을 때 호출되는 콜백
        public delegate void NewclientHandler(Socket client_socket, object sender);
        ...
        void do_listen()
        {
            // accpet 처리 제어를 위해 이벤트 객체를 생성합니다.
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    // 리턴값을 확인하여 분기시켜야 합니다. 
                    pendig = listen_socket.AcceptAsync(this.accept_args);
                }
                catch (Exception e)
                {
